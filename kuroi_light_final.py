# kuroi_navigator_lengkap_v2.py - Perbaikan Final untuk Jarak & Unit
...
# Semua fungsi di sini tidak berubah, kecuali get_pedestrian_route_with_instructions
...
#        print("Mencari sinyal dari modul GPS fisik...")
#        for _ in range(5):
#            report = session.next()
#            if report['class'] == 'TPV' and hasattr(report, 'mode') and report.mode >= 2:
#                lat, lon = report.lat, report.lon
#                print(f"Sukses! GPS fisik mendapatkan sinyal di {lat},{lon}")
#                return f"{lat},{lon}"
#        print("GPS fisik terhubung, tetapi belum mendapatkan sinyal (NO FIX).")
#        return None
#    except Exception:
#        print("GPS fisik tidak terdeteksi.")
#        return None
...
def get_best_available_location():
    """Karena modul GPS dikomentari, langsung menggunakan IP Geolocation."""
    print("\nMenggunakan IP Geolocation sebagai sumber lokasi utama.")
    location = get_location_from_ip()
    return location
def get_routable_location(lat_lon_str, maps_key):
    print(f"Mencari jalan terdekat untuk koordinat: {lat_lon_str}...")
    try:
        url = f"https://atlas.microsoft.com/search/address/reverse/json?api-version=1.0&query={lat_lon_str}&subscription-key={maps_key}"
    ...
    try:
        ...
        if data.get('routes'):
            ...
            if route.get('guidance') and route['guidance'].get('instructions'):
                ...
                # --- PERBAIKAN UTAMA ADA DI DALAM LOOP DI BAWAH INI ---
                for i, step in enumerate(instructions):
                    message = step.get('message', 'Instruksi tidak tersedia')
                    distance = 0
                    
                    # 1. Mengembalikan logika perhitungan jarak yang benar
                    ...
                    # 2. Mengubah format 'm' menjadi 'meter'
                    unit = "meter"
...
if __name__ == "__main__":
    try:
        ...
        if input_pengguna_suara:
            ...
            if raw_start_coords:
                # Ganti "Monas" atau input lain menjadi key kamus yang valid
                # Logika sederhana: cari kata kunci di dalam input suara
                ...
                if nama_panggilan_tujuan is None:
                    # Jika tidak ada di kamus, coba gunakan input suara langsung
                ...
                # Jika tidak ada di kamus, nama lengkap = nama panggilan
                if nama_lengkap_tujuan is None:
                    nama_lengkap_tujuan = nama_panggilan_tujuan

                clean_start_coords = get_routable_location(raw_start_coords, maps_key)
                if clean_start_coords:
                    ...
                    if clean_dest_coords:
                        get_pedestrian_route_with_instructions(clean_start_coords, clean_dest_coords, maps_key, speech_key, speech_region)
